    ...
    <div className={`relative w-full transition-colors duration-1000 ease-in-out${exploreMode ? ' h-screen overflow-hidden' : ''}`} style={{ backgroundColor: theme.background }}>
      ...
      <div className="relative h-screen w-full overflow-hidden">
        ...
        <div className={`transition-opacity duration-500 ${exploreMode ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
          <HeroOverlay accentColor={theme.accent} toolbarColor={theme.toolbar} foregroundRgb={theme.foregroundRgb} />
        </div>

        {/* Desktop: Social links — top left */}
        <div className={`hidden md:flex fixed top-7 left-8 z-50 items-center gap-5 transition-opacity duration-500 ${exploreMode ? 'opacity-0 pointer-events-none' : 'opacity-100'}`} style={{ color: theme.accent }}>
      ...
      <div className={`md:hidden relative z-20 px-6 py-16 flex flex-col gap-10 ${exploreMode ? 'hidden' : ''}`} style={{ backgroundColor: theme.background }}>
        <ScrollReveal>
          <p className="text-base font-sans leading-relaxed" style={{ color: `rgba(${theme.foregroundRgb}, 0.7)` }}>
        ...
        <ScrollReveal delay={150}>
          <span className="text-base font-sans leading-relaxed block" style={{ color: `rgba(${theme.foregroundRgb}, 0.7)` }}>
        ...
        <ScrollReveal delay={300}>
          <footer className="pt-10 pb-6 text-center" style={{ textWrap: 'balance' } as React.CSSProperties}>
            <p className="text-sm font-sans leading-relaxed" style={{ color: `rgba(${theme.foregroundRgb}, 0.4)` }}>
